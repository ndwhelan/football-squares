...
def build_squares(teams: Tuple[str, str], players: Iterable[Tuple[str, int]]) -> None:
    ...
    side_score_values: List[int] = SCORE_VALUES.copy()

    random.shuffle(top_score_values)
    random.shuffle(side_score_values)

    top_score_values.insert(0, " ")  # Create a blank corner square

    for x in range(10):
        # Insert side scores
        player_array_split[x].insert(0, side_score_values[x])
